...
int main(int argc, char **argv) {
  ...
  users[2].name = (char *)"aman";
  users[2].ip = (char *)"10.42.0.180\0";
  users[2].port_no = 9000;

  users[4].name = (char *)"server";
  users[4].ip = (char *)"10.5.18.69";
      ...
      for(i=0;i<=max_fd;i++){
        if(FD_ISSET(i,&read_fds))
        {
                  if(i==parentfd)
                  {
                        ...
                        cout<<"CHILD FD: "<<childfd<<endl;
                        cout<<"225"<<endl;
                        cout<<"IP "<<newtemp.user.ip<<" port number: "<<newtemp.user.port_no<<endl;
                        for(int q=0;q<5;q++)
                        {
                            //&& users[q].port_no==newtemp.user.port_no
                            cout<<"IP "<<users[q].ip<<endl;
                        ...
                        printf("New connection , socket fd is %d , ip is : %s , port : %d , name:%s\n" , childfd , inet_ntoa(clientaddr.sin_addr) , ntohs(clientaddr.sin_port),newtemp.user.name);
                        cout<<"239"<<endl;
