...
int appRecv(char *buffer){
	/* calls recvbuffer_handle and gets data from there
	is blocked if no data in buffer */
	//int fd = creat("ReceivedData.txt",0666);
  //char *buffer = (char *)malloc(sizeof(char)*RECV_BUFFER);
  if(recv_window_free< RECV_BUFFER){
    for(int i=0;i<RECV_BUFFER-recv_window_free;i++){
      buffer[i]=receiverBuffer[i];
    }
  }
